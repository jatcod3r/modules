...
variable "distro" {
    ...
    validation {
        condition = contains(["ubuntu", "debian", "rhel", "suse", "al2023"], var.distro)
        error_message = "'distro' invalid. Must be either 'ubuntu', 'debian', 'rhel', 'suse', or 'al2023'."
    }
}
...
variable "architecture" {
    ...
    validation {
        ...
        error_message ="'architecture' invalid. Must be either 'amd64' or 'arm64'"
    }
}
...
locals {
    ami_arch = var.architecture == "amd64" ? "x86_64" : var.architecture
    ami_name = {
        ubuntu = "ubuntu/images/hvm-ssd-gp3/ubuntu-*"
        debian = "debian-*"
        rhel = "RHEL-10.0.0_HVM-*",
        suse = "suse-sles-*"
        al2023 = "al2023-*"
    }
